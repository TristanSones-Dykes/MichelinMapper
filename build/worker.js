...
// shared/classifier.ts
var GASTRONOMIC_RULES = [
  {
    name: "Seafood",
    type: "gastronomic",
  ...
  },
  {
    name: "Dessert",
    type: "gastronomic",
    keywords: ["chocolate", "souffle", "souffl\xE9", "tart", "ice cream", "sorbet", "millefeuille"]
  },
  {
    name: "Japanese",
    type: "cuisine",
  ...
  },
  {
    name: "French",
    type: "cuisine",
    keywords: ["foie gras", "beurre blanc", "confit", "terrine", "veloute", "velout\xE9"]
  },
  {
    name: "Luxury",
    type: "gastronomic",
...
function addTag(tags, name, type, confidence) {
...
async function fetchRestaurants(sourceUrl) {
...
async function upsertRestaurant(env, restaurant) {
...
async function upsertDish(env, restaurantId, source, dish) {
  ...
  const row = await env.DB.prepare("SELECT id FROM dishes WHERE source = ? AND source_id = ?").bind(source, dish.sourceId).first();
  if (!row) {
    throw new Error(`Dish upsert failed for ${source}:${dish.sourceId}`);
  }
  return row.id;
}
async function replaceDishTags(env, dishId, tags) {
  ...
  for (const tag of tags) {
    ...
    const row = await env.DB.prepare("SELECT id FROM tags WHERE slug = ?").bind(tag.slug).first();
    if (!row) {
...
async function createIngestionRun(env, source) {
...
async function finishIngestionRun(env, runId, status, stats) {
  await env.DB.prepare(
...
async function failIngestionRun(env, runId, error) {
  await env.DB.prepare(
    `UPDATE ingestion_runs
    SET status = 'failed',
      error_message = ?,
      finished_at = datetime('now')
    WHERE id = ?`
  ).bind(error instanceof Error ? error.message : "Unknown crawler failure", runId).run();
}

// worker/src/api/dishQuery.ts
var AWARD_TYPES = /* @__PURE__ */ new Set([
  "3-star",
  "2-star",
  "1-star",
  "bib-gourmand",
...
var SORTS = /* @__PURE__ */ new Set(["newest", "name", "restaurant"]);
function parseDishQuery(url) {
  const page = clampInteger(url.searchParams.get("page"), 1, 1, 1e4);
  ...
  return {
    ...AWARD_TYPES.has(awardTypeCandidate) ? { awardType: awardTypeCandidate } : {},
    limit,
    offset: (page - 1) * limit,
    ...search ? { search } : {},
